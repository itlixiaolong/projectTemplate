...
export default {
  data () {
    return {
      ...
      showValue: false,
      searchIpt: '',
      queryUrl: 'project/listAllProject'
    }
  },
  props: {
    value: {
  ...
  methods: {
    ...
    searchEvt () {
      this.queryUrl = 'project/listAllProject'
      if ($.trim(this.searchIpt)) {
        this.queryUrl += '?projectName=' + $.trim(this.searchIpt)
        this.getAllBusiness()
      }
    },
    selectedBusiness () {
      let params = this.rowData.code
      this.Api.httpGet('project/listAllDepProjectByDepCode?depCode=' + params, '')
        .then(res => {
    ...
    saveBusiness () {
      ...
      this.selectData.map(item => {
        let obj = item.projectCode
        ids.push(obj)
      })
      let params = {
        depCode: this.rowData.code,
        list: ids
      }
      this.Api.httpPost('depProject/batchInsertAllDepProject', params)
        .then(response => {
